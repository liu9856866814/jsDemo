...
// => 现在有一个bug，第一次我点击了上架时间，上架时间按照升序排列了（全局变量flag变成了1），接下来我点击 价格，此时应该按
// 照价格升序排，但是flag此时是1，点击过后就成-1，所以结果就是按照价格降序排列。原因是三个q标签修改的是同一个flag，造成了
// 排序混乱，所以我们采用自定义属性的方式让每个a标签私有一个flag
...
for (let i = 0; i < linkList.length; i++) {
  ...
  linkList[i].onclick = function () {
    ...
    // => 如果你点击某一个a标签的是，想让列表按照当前维度升序排序，就要保证当前a标签的flag是-1；所以我们点击当前a标签的时候，
    // 把其他两个a标签的flag重置成-1；
