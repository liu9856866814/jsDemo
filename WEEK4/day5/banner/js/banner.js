...
$.ajax({
  ...
  success(data){ // ajax无论异步还是同步执行如果成功都会执行success; 但是异步执行时,$ajax()后面的代码不会等着ajax结束.
    // console.log(data);
    //在这里执行绑定数据的方法
    bindHTML(data); // bindHTML()方法结束后,$focusList重新赋值,不再是null
    handleFocus(); // 上面执行过bindHTML方法后,$focusList有值了,再绑定事件不会报错;
  }
});
...
// handleFocus(); 因为ajax是异步的,这里执行handleFocus()是同步执行,此时$focusList还是null,再给null绑定事件就会报错;
